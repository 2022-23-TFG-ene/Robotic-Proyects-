...
double MMA7361x; // Datos a leer desde el giroscopio
double MMA7361y;
double MMA7361z;
...
double MMA7361xFinalCorregirSinMapear = 0; // Valores a corregir como diferencia en ángulo del plano, sin mapear a los ángulos reales considerados en los motores
...
// Esta parte viene de aplicar una solución según em modelo de SCARA.
  ...
  Serial.print(F(" - Teta5pgrados: ")); Serial.println(Teta5pgrados);
    //Teta1pradianes = Teta5pradianes;
    //Teta2pgrados = Teta5pradianes + MMA7361xFinalCorregir;
...
void calcularPlanoInclinado() {
  ...
  if (contadorMMA7361General == 20) { // Se calculan los Offset iniciales. No se usan las primeras lecturas para que se estabilice la desviación
    ...
    MMA7361xAnterior = MMA7361xInicial;
    MMA7361yAnterior = MMA7361yInicial;
  ...
  if (contadorMMA7361Corregir < 100) { // Inicialmente vale 100 para que no se ejecute esta parte hasta calcular el valor de referencia inicial (MMA7361xInicial).
    ...
    if (contadorMMA7361Corregir == 15) { // No se usan las primeras lecturas para que se estabilice la desviación
      ...
      if (((abs(MMA7361xFinalCorregirSinMapear) > 4) or (abs(MMA7361yFinalCorregirSinMapear) > 4)) and (corregir == 0)) { // Si se produce una primera desviación, se comprobará por segunda vez
        ...
        MMA7361yFinalCorregirMapeado = MMA7361yAnterior + MMA7361yFinalCorregirMapeado; // Se puede usar el anterior porque el inicial lo mapea a 0º. De no ser así se definiría a 0.
