...
///double distanciaPasoMaximo = 6.0; // Distancia máxima (mm) que puede ser alcanzada en un paso no forzado: (L1+L2)*0,4
///double cabeceo = 0; // 20; Distancia a adelantar los brazos para evitar el cabeceo al mover los brazos hacia atrás al mismo tiempo.
///double distanciaBrazoMaximo = 25.0 ;//30; // Distancia máxima que puede ser alcanzada con el brazo: (L1+L2)*0,8
///double distanciaMaximaRectaParalela = 30; // Máxima distancia del cuerpo a la recta que sigue el paso en paralelo al sentido de movimiento: (LH+L1)*0,9 = 13
/////double alturaLim = 0.1; // Límite de altura sobre el robot a partir del cual se utilizará también el segundo motor (ω) por ser un punto no alcanzable. 
...
/////double omegaGrados; double omegasec; 
...
// Esta parte viene de aplicar una solución según em modelo de SCARA.
    ...
    Teta4pgrados = radianesAGrados(Teta4pradianes); // +- 
    Teta5pradianes = Teta1pradianes;
    Teta5pgrados = Teta1pgrados;
...
//tiempoPausaDivisorMovimientos = 18;
    ...
    double DesfaseEquilibrioLateralDosPiernas = 6; // Movimiento lateral para equilibrar el Centro de Gravedad sobre una única pierna.
    double DesfaseEquilibrioLateralUnaPierna = 6; // Movimiento lateral para equilibrar el Centro de Gravedad sobre una única pierna.
...
//--------------- Paso 1
  ...
  for (indpaso1 = 1; indpaso1 <= 3; indpaso1  += 1) { // SE PONDRÁ A 1 CUANDO CAMINE DE FORMA CONSTANTE YA QUE SE REPETIRÁ MEDIANTE loop().
