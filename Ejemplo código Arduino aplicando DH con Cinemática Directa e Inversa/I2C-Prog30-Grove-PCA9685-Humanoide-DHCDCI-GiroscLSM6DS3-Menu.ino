...
#define DISPLAY_INTERVAL 100  // 100 - 300   // Interval between pose displays
unsigned long lastDisplay;
unsigned long lastRate;
int sampleCount;

double LSM6DS3x = 0; // Datos a leer desde el giroscopio
double LSM6DS3y = 0;
double LSM6DS3z = 0;
double LSM6DS3contadorGeneral = 0; // Contador para calcular el Offset medio inicial
double LSM6DS3ContadorCorregir = 100; // Contador para tomar valores medios posteriores y compararlos con el inicial
double LSM6DS3varXOffset = 0; // OFfset inicial o de referencia
double LSM6DS3varYOffset = 0;
double LSM6DS3xCorregir = 0; // Diferencias entre el Offset inicial y una medida media posterior
double LSM6DS3yCorregir = 0;
...
double LSM6DS3xFinalCorregirSinMapear = 0;// Valores a corregir como diferencia en ángulo del plano
double LSM6DS3yFinalCorregirSinMapear = 0;
double LSM6DS3XAnterior = 0; // Valor de los ángulos mapeados en el anterior equilibrado para hacer la modificación desde ese ángulo y no desde la posición inicial, sin inclianción en el plano.
double LSM6DS3YAnterior = 0;
int corregir = 0; // Variable para determinar si se mantiene un desequilibrio tras una primera medida.
///////////////////////////

double imprimir0 = 3; // Para imprimir o no las variables a analizar. Con este valor, no se imprimiría nada.
...
double msec0[] =             {412, 388, 393, 390, 403,   398, 398, 398,   397, 408, 413, 405, 380,   398, 398, 398}; // Pulsos de cada motor equivalente a "0" grados.
double msecMin[] =           {256, 232, 237, 233, 247,   242, 242, 242,   241, 252, 257, 244, 230,   242, 242, 242}; // Mínimos pulsos de cada motor. Es equivalente al array msecMin[].
double msecMax[] =           {568, 544, 549, 546, 559,   554, 554, 554,   553, 564, 571, 556, 542,   554, 554, 554}; // Máximos pulsos de cada motor. Es equivalente al array msecMax[].
...
double posReposoDePie[] =    {412, 388, 393, 390, 403,   398, 398, 398,   397, 408, 413, 405, 380,   398, 398, 398}; // Pulsos de cada motor para reposo (de pie).
...
int Option = 0;
int optionRecuperacion = 1; // Porcentaje de recuperación en cada movimiento (0=3 ; 1:100% (.../1) ; 2:50% (.../2); 3:33% (.../3); 4:25% (.../4);...)
int claveMenu = 0; // Para que en el menú no siga escribiendo el texto constantemente, sino sólo un "." en cada iteración del loop.
int claveImprimir = 0; // Para introducir la cantidad de información que se quiera ver por pantalla.
//****************************************************************************************
...
// Esta parte viene de aplicar una solución según em modelo de SCARA.
    ...
    //Teta4pgrados = radianesAGrados(Teta4pradianes) - (MPU9250xFinalCorregirSinMapear/6); // +- // 6 - porque se va acercando a la solución de forma iterativa - Se corrige el ángulo según el giroscopio.
    Teta4pgrados = radianesAGrados(Teta4pradianes) - (LSM6DS3xFinalCorregirSinMapear/optionRecuperacion); // +- // /2 - porque se va acercando a la solución de forma iterativa - Se corrige el ángulo según el giroscopio.
    Teta5pradianes = Teta1pradianes;
    if (brazoN == 1) {
      //Teta5pgrados = Teta1pgrados - (MPU9250yFinalCorregirSinMapear); // /2 - Se corrige el ángulo según el giroscopio 
      Teta5pgrados = Teta1pgrados + (LSM6DS3yFinalCorregirSinMapear/optionRecuperacion); // /2 - Se corrige el ángulo según el giroscopio 
    } else if (brazoN == 2) {
      //Teta5pgrados = Teta1pgrados + (MPU9250yFinalCorregirSinMapear); // /2 - Se corrige el ángulo según el giroscopio      
      Teta5pgrados = Teta1pgrados - (LSM6DS3yFinalCorregirSinMapear/optionRecuperacion); // /2 - Se corrige el ángulo según el giroscopio      
    }    
...
void posAgachado() {
  ...
  Serial.println(F("\n  El robot se agacha hasta su posición de inicio de caminar"));
  puntoFinalAbsoluto(Long5i, Long6i, Long7i, piernaN1); 
  puntoFinalAbsoluto(Long5i, Long6i, Long7i, piernaN2); 
  ejecutarParalelo();
  //delay(tiempoEntreMovimientos); 
...
void corregircentroGravedad() {
  ...
  diferenciaPiesCorregirEquilibrio =  - (distanciaPiesEquilibrio * tan(gradosARadianes(LSM6DS3yFinalCorregirSinMapear)));
  ...
  puntoFinalAbsoluto(Long5i, Long6i, (Long7i+(diferenciaPiesCorregirEquilibrio/optionRecuperacion)), piernaN1); // /2 - Se modifica la mitad de la altura en cada pierna.??????
  puntoFinalAbsoluto(Long5i, Long6i, (Long7i-(diferenciaPiesCorregirEquilibrio/optionRecuperacion)), piernaN2); 
  ejecutarParalelo();
  //delay(tiempoEntreMovimientos); 
  diferenciaPiesCorregirEquilibrio = 0;
  LSM6DS3xFinalCorregirSinMapear = 0;
  LSM6DS3yFinalCorregirSinMapear = 0;

  return; // Punto de verificación de posición y par de los motores
}

void acelerometerLSM6DS3() {
      LSM6DS3x = radianesAGrados(myIMU.readFloatAccelX());
      LSM6DS3y = radianesAGrados(myIMU.readFloatAccelY());
      LSM6DS3z = radianesAGrados(myIMU.readFloatAccelZ());
      
      LSM6DS3contadorGeneral = LSM6DS3contadorGeneral + 1;
      if ((LSM6DS3contadorGeneral >= 80) and (LSM6DS3contadorGeneral < 85)) {
        LSM6DS3varXOffset = LSM6DS3varXOffset + LSM6DS3x; 
        LSM6DS3varYOffset = LSM6DS3varYOffset + LSM6DS3y;
        if (imprimir0 == 1) {     
          Serial.print(F("\n **** LSM6DS3contadorGeneral: ")); Serial.print(LSM6DS3contadorGeneral); 
          Serial.print(F(" - Roll: ")); Serial.print(LSM6DS3x); 
          Serial.print(F(" - Pitch: ")); Serial.print(LSM6DS3y); 
          Serial.print(F(" - Yaw: ")); Serial.print(LSM6DS3z); 
        }
      }
      if ((LSM6DS3contadorGeneral == 85)) {
        LSM6DS3varXOffset = LSM6DS3varXOffset/5;  // Se calculan los Offset iniciales
        LSM6DS3varYOffset = LSM6DS3varYOffset/5;
        LSM6DS3ContadorCorregir = 0;
        //Serial.print(F("---- LSM6DS3contadorGeneral: ")); Serial.print(LSM6DS3contadorGeneral); 
        if ((imprimir0 == 0) or (imprimir0 == 1)) {     
          Serial.print(F("\nRoll: ")); Serial.print(LSM6DS3x); 
          Serial.print(F(" - Pitch: ")); Serial.print(LSM6DS3y); 
          Serial.print(F(" - Yaw: ")); Serial.print(LSM6DS3z); 
          Serial.print(F(" - LSM6DS3varXOffset: ")); Serial.print(LSM6DS3varXOffset); 
          Serial.print(F(" - LSM6DS3varYOffset: ")); Serial.println(LSM6DS3varYOffset); 
        }
      } 
      if (LSM6DS3ContadorCorregir <100) {
        LSM6DS3ContadorCorregir = LSM6DS3ContadorCorregir + 1;
        if ((LSM6DS3ContadorCorregir >= 5) and (LSM6DS3ContadorCorregir < 10)) { // No se usan las primeras lecturas para que se estabilice la desviación
          LSM6DS3xCorregir = LSM6DS3xCorregir + LSM6DS3x; 
          LSM6DS3yCorregir = LSM6DS3yCorregir + LSM6DS3y;
        }
        if (LSM6DS3ContadorCorregir == 10) { // Se calculan los Offset iniciales. No se usan las primeras lecturas para que se estabilice la desviación
          LSM6DS3xCorregir = LSM6DS3xCorregir/5; // Valor medio de la desviación producida cada 5 lecturas 
          LSM6DS3yCorregir = LSM6DS3yCorregir/5;
          LSM6DS3xFinalCorregirSinMapear = LSM6DS3xCorregir-LSM6DS3varXOffset; // Se compara la posición actual con la del Offset para comprobar si hay una desviación.
          LSM6DS3yFinalCorregirSinMapear = LSM6DS3yCorregir-LSM6DS3varYOffset;      
          if (((abs(LSM6DS3xFinalCorregirSinMapear) > 4) or (abs(LSM6DS3yFinalCorregirSinMapear) > 4)) and (corregir == 0)) { // Si se produce una primera desviación, se comprobará por segunda vez
            corregir = 1;
          } else if (((abs(LSM6DS3xFinalCorregirSinMapear) <= 4) and (abs(LSM6DS3yFinalCorregirSinMapear) <= 4)) and (corregir == 1)) { // Si no se mantiene la desviación, se considera un fallo
            corregir = 0;
          } else if (((abs(LSM6DS3xFinalCorregirSinMapear) > 4) or (abs(LSM6DS3yFinalCorregirSinMapear) > 4)) and (corregir == 1)) { // Si se mantiene la desviación, se considera que es real
            if (imprimir0 == 1) {     
              Serial.print(F("\n........................................................................................................"));
              Serial.print(F("\nLSM6DS3x: ")); Serial.print(LSM6DS3x);
              Serial.print(F(" - LSM6DS3y: ")); Serial.print(LSM6DS3y);
              Serial.print(F("\nOffset-x de referencia (LSM6DS3varXOffset): ")); Serial.print(LSM6DS3varXOffset); // Offset de referencia
              Serial.print(F(" - Offset-y de referencia (LSM6DS3varYOffset): ")); Serial.print(LSM6DS3varYOffset);
              Serial.print(F("\nLSM6DS3xCorregir: ")); Serial.print(LSM6DS3xCorregir); 
              Serial.print(F(" - LSM6DS3yCorregir: ")); Serial.println(LSM6DS3yCorregir);   
              Serial.print(F("\nLSM6DS3xFinalCorregirSinMapear: ")); Serial.print(LSM6DS3xFinalCorregirSinMapear); 
              Serial.print(F(" - LSM6DS3yFinalCorregirSinMapear: ")); Serial.print(LSM6DS3yFinalCorregirSinMapear);   
            }
            LSM6DS3xFinalCorregirSinMapear = LSM6DS3XAnterior + LSM6DS3xFinalCorregirSinMapear; // Se guardan los valores anteriores ya que son acumulativos. Se corrige sobre la corrección anterior y no desde 0.
            LSM6DS3yFinalCorregirSinMapear = LSM6DS3YAnterior + LSM6DS3yFinalCorregirSinMapear; // LSM6DS3XAnterior debe ser inicialmente 0 puesto que en reposo se encuentra en un plano horizontal.
            LSM6DS3XAnterior = LSM6DS3xFinalCorregirSinMapear;
            LSM6DS3YAnterior = LSM6DS3yFinalCorregirSinMapear;
            if (imprimir0 == 1) {     
              Serial.print(F("\nLSM6DS3XAnterior: ")); Serial.print(LSM6DS3XAnterior); 
              Serial.print(F(" - LSM6DS3YAnterior: ")); Serial.print(LSM6DS3YAnterior); 
              Serial.print(F("\nLSM6DS3xFinalCorregirSinMapear - Final: ")); Serial.print(LSM6DS3xFinalCorregirSinMapear); 
              Serial.print(F(" - LSM6DS3yFinalCorregirSinMapear - Final: ")); Serial.println(LSM6DS3yFinalCorregirSinMapear);
            }
...
void serialEvent() {
  while (Serial.available()) {
    ...
    inputString += inChar;
    if (inChar == '\n') { // Si se presiona un enter, no permite meter más caracteres y se asigna la opción 
      Serial.print(F("\n *** Opción: "));Serial.println(inputString);
      stringComplete = true;
    }
  }
}
...
double Menu1() {
  ...
  Serial.print(F("\n\t 2 - Prueba de equilibrio sobre un plano en movimiento con un porcentaje de recuperación por determinar. "));
...
double Menu2() {
  //Serial.println("************************* Pasa por Menu2");
  // Menú de Pruebas de movimiento.  
  if (Option == 1) { // Prueba de movimientos
    ...
    claveMenu = 0;
    imprimir0 = 3; // Se vuelve a la opción 3 (Espera la elección de una opción)
    ...
    int bucle = 0;
    while (bucle == 0){
      acelerometerLSM6DS3(); // Función para calcular la inclinación del plano en ejes X,Y para su corrección
    }
    setServos01234(); 
    Option = 0;
    claveMenu = 0;
...
void loop() {
  ...
  if (Option == 0) {
    ...
    if (stringComplete) {
      ...
      if ((StrToDouble(inputString) == 1) or (StrToDouble(inputString) == 2)) {
  ...
  if ((Option == 1) and (claveImprimir == 0)) { // Se define el número de pasos.
    if (claveMenu == 0) {
  ...
  if ((Option == 2) and (claveImprimir == 0)) { // Se define el porcentaje de recuperación.
    if (claveMenu == 0) {
      Serial.println(F("\n Hay que introducir el porcentaje de recuperación en cada movimiento (0 - estándar (2); 1 - 100% ; 2 - 50% ; 3 - 33%; 4 - 25%; 5 - 20%; .... "));
      claveMenu = 1;
    } else if (claveMenu == 1) {
      Serial.print(F("."));        
    }
    if (stringComplete) {
      //Serial.println(inputString);
      String otionString = inputString; 
      if ((StrToDouble(otionString) >= 0) and (StrToDouble(otionString) < 100)) {
        optionRecuperacion = StrToDouble(otionString);
      }
      if (optionRecuperacion == 0) {
        optionRecuperacion = 3;
      }
      inputString = "";
      stringComplete = false;
      claveMenu = 0;
  ...
  if ( (((Option == 2) and ((optionRecuperacion >= 0) and (optionRecuperacion < 10))) or ((Option == 1) and (numPasos != 0))) and (claveImprimir == 100) ) { // Se define cuánta información saldrá por pantalla
    if (claveMenu == 0) {
      Serial.println(F("\n Hay que introducir la cantidad de información que se verá por pantalla (0 - Detalles básicos ; 1 - Detalles para el programador ; 2 - Sólo movimientos y puntos de cada pie .... "));
      claveMenu = 1;
    } else if (claveMenu == 1) {
      Serial.print(F("."));        
    }
    if (stringComplete) {
      String otionString = inputString; 
      if ((StrToDouble(otionString) == 0) or (StrToDouble(otionString) == 1) or (StrToDouble(otionString) == 2)) {
        Serial.print(F("\n Se ejecuta el Menú."));
        imprimir0 = StrToDouble(otionString);
      }
      if ((imprimir0 == 0) or (imprimir0 == 1) or (imprimir0 == 2)) {
        Menu2(); // Define las opciones del menú y las ejecuta.
      }    
      inputString = "";
      stringComplete = false;
      claveMenu = 0; // Para poder volver a definir otra opción añadiendo "." a la línea de petición de opción
      claveImprimir = 0; // Para poder volver a definir otra opción eligiendo la información a ver
    } 
  }   
}
